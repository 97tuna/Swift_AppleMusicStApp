...
class PlayerViewController: UIViewController {
    ...
    @IBAction func togglePlayButton(_ sender: UIButton) {
        // TODO: 플레이버튼 토글 구현, 버튼 일시정지, 재생 변경
...
extension PlayerViewController {
    ...
    func updatePlayButton() {
        ...
        if simplePlayer.isPlaying {
            let configuaration = UIImage.SymbolConfiguration(pointSize: 40) // 아이콘 이미지 크기
            let image = UIImage(systemName: "pause.fill", withConfiguration: configuaration)
            playControlButton.setImage(image, for: .normal)
        } else {
            let configuaration = UIImage.SymbolConfiguration(pointSize: 40) // 아이콘 이미지 크기
            let image = UIImage(systemName: "play.fill", withConfiguration: configuaration)
            playControlButton.setImage(image, for: .normal)
        }
    }
}
